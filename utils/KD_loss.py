...
class DistributionLoss(loss._Loss):
    ...
    def forward(self, model_output, real_output, reduce = True):

        #self.size_average = True

        # Target is ignored at training time. Loss is defined as KL divergence
        # between the model output and the refined labels.
        if real_output[0].requires_grad:
        ...
        for i in range(len(real_output)):
            real_output_soft += F.softmax(real_output[i], dim=1).detach()
        real_output_soft = real_output_soft.div(len(real_output))
        '''
        real_output_soft = F.softmax(real_output, dim=1).detach()
